...
class SKToolsSunriseSunsetCalculator {
    ...
    private static void calculateTime(double latitude, double longitude, double zenith, boolean calculateSunrise) {
        ...
        double N3 = (1 + Math.floor((currentYear - 4.0
                * Math.floor(currentYear / 4.0) + 2.0) / 3.0));
        ...
        double sunLongitude = meanAnomaly
                ...
                + (0.020 * Math.sin(2 * Math.toRadians(meanAnomaly))) + 282.634;
        ...
        double sunSinDeclination = 0.39782 * Math.sin(Math
                .toRadians(sunLongitude));
        double sunCosDeclination = Math.cos(Math.asin(sunSinDeclination));

        // calculate the Sun's local hour angle

        double cosLocalHour = (Math.cos(Math.toRadians(zenith)) - (sunSinDeclination * Math
                .sin(Math.toRadians(latitude))))
                / (sunCosDeclination * Math.cos(Math.toRadians(latitude)));
        ...
        double localMeanTime = localHour + sunRightAscension
        ...
        int localOffset = getCurrentTimezoneOffset();
        ...
        if (calculateSunrise) {
            ...
            Log.d(TAG, "Sunrise : " + SKToolsDateUtils.AUTO_NIGHT_SUNRISE_HOUR + ":" + SKToolsDateUtils
                    .AUTO_NIGHT_SUNRISE_MINUTE);
        } else {
            SKToolsDateUtils.AUTO_NIGHT_SUNSET_HOUR = hour;
            SKToolsDateUtils.AUTO_NIGHT_SUNSET_MINUTE = minute;
            Log.d(TAG, "Sunset : " + SKToolsDateUtils.AUTO_NIGHT_SUNSET_HOUR + ":" + SKToolsDateUtils
                    .AUTO_NIGHT_SUNSET_MINUTE);
        }
    }
