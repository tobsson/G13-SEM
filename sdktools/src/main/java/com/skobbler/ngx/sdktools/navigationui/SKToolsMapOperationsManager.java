    ...
    private int getCorrectMapStyleForDriveModeWhenAutoNightIsOn(boolean autoNightIsOn) {
        if (autoNightIsOn) {
            if (SKToolsLogicManager.lastUserPosition != null) {
                SKToolsAutoNightManager.getInstance().calculateSunriseSunsetHours(SKToolsLogicManager
                        .lastUserPosition.getLatitude(), SKToolsLogicManager.lastUserPosition.getLongitude());
                if (SKToolsAutoNightManager.getInstance().isCurrentTimeInSunriseSunsetLimit()) {
                    return DAY_STYLE;
                } else {
                    return NIGHT_STYLE;
                }
